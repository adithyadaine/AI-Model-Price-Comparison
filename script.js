    ...
    providerGroups.forEach((groupDiv, index) => {
        if (!groupDiv.classList.contains('expanded')) {
            ...
            if (providerTitle) {
                ...
                console.log(`Expanded group ${index + 1}:`, providerTitle.textContent);
            } else {
                console.warn(`Provider title not found for group ${index + 1}.`);
            }
        } else {
            console.log(`Group ${index + 1} (${groupDiv.querySelector('.provider-title')?.textContent}) already expanded.`);
        }
    });
